...
for (const [key, result] of Object.entries(snapshots)) {
  ...
  assert(Number.isFinite(result.last.cumulativeCashValue), `${key}: cumulative cash value is not finite`);
  assert(Number.isFinite(result.last.cumulativeShadowValue), `${key}: cumulative shadow value is not finite`);
  ...
  assert(result.last.cumulativeEasySavingsValue >= result.first.easySavingsValue, `${key}: easy savings did not accumulate`);
  assert(result.last.cumulativeLifeProjectValue >= 0, `${key}: life/project value went negative`);
}
...
assert(
  snapshots.household_subscription_audit.first.easySavingsValue / snapshots.household_subscription_audit.first.activeBefore > snapshots.household_subscription_audit.last.easySavingsValue / snapshots.household_subscription_audit.last.activeBefore,
  'first audit savings per active household should taper after the easy stock is consumed'
);
assert(snapshots.household_subscription_audit.last.cumulativeEasySavingsValue > snapshots.household_subscription_audit.last.cumulativeAdminValue, 'household baseline should preserve a visible first-audit value wedge');
assert(snapshots.personal_to_work_agent_bridge.last.marketWorkValue > snapshots.personal_to_work_agent_bridge.first.marketWorkValue, 'work bridge market matching should improve as the network matures');
...
const summary = Object.fromEntries(Object.entries(snapshots).map(([key, result]) => [key, {
  ...
  cashValue: Number(result.last.cumulativeCashValue.toFixed(0)),
  shadowValue: Number(result.last.cumulativeShadowValue.toFixed(0)),
